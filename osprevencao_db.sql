...
CREATE TABLE `cidade` (
  ...
  CONSTRAINT id_estado_fk FOREIGN KEY (id_estado_fk) REFERENCES estado(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `cep` (
  ...
  CONSTRAINT estado_fk FOREIGN KEY (estado_fk) REFERENCES estado(id),
  CONSTRAINT id_cidade_fk FOREIGN KEY (id_cidade_fk) REFERENCES cidade(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `empresa` (
  ...
  CONSTRAINT id_tipo_cadastro_fk FOREIGN KEY (id_tipo_cadastro_fk) REFERENCES tipocadastro(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `endereco` (
  ...
  CONSTRAINT id_Empresa_fk FOREIGN KEY (id_Empresa_fk) REFERENCES empresa(id),
  CONSTRAINT id_Logradouro_fk FOREIGN KEY (id_Logradouro_fk) REFERENCES logradouro(id),
  CONSTRAINT id_TipoEndereco_fk FOREIGN KEY (id_TipoEndereco_fk) REFERENCES tipoendereco(id),
  CONSTRAINT id_cep_fk FOREIGN KEY (id_cep_fk) REFERENCES cep(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `orcamento` (
  ...
  CONSTRAINT id_status_fk FOREIGN KEY (id_status_fk) REFERENCES status_orcamento(id)
...
CREATE TABLE `listamateriais` (
  ...
  CONSTRAINT id_Orcamento_fk FOREIGN KEY (id_Orcamento_fk) REFERENCES orcamento(id),
  CONSTRAINT id_Produto_fk FOREIGN KEY (id_Produto_fk) REFERENCES produto(id)
 ...
 CREATE TABLE nota(
  ...
  CONSTRAINT produto_id_fk FOREIGN KEY (produto_id_fk) REFERENCES produto(id),
  CONSTRAINT empresaid_fk FOREIGN KEY (empresaid_fk) REFERENCES empresa(id)
...
CREATE VIEW `produtos_compra` AS
    SELECT 
        `es`.`quantidade` AS `quantidade`,
        `p`.`id` AS `id`,
        `e`.`NomeEmpresa` AS `NomeEmpresa`,
        `p`.`produto` AS `produto`,
        `nt`.`ValorUnitario` AS `ValorUnitario`,
        `p`.`descricao` AS `descricao`,
        `nt`.`numero_nota` AS `numero_nota`,
        `nt`.`valor_nota` AS `valor_nota`,
        `nt`.`data_compra` AS `data_compra`
    FROM
        `produto` `p`
        JOIN `nota` AS `nt` ON `nt`.`produto_id_fk` = `p`.`id`
        JOIN `empresa` AS `e` ON `nt`.`empresaid_fk` = `e`.`id`
        JOIN `estoque` AS `es` ON `es`.`id_produto_fk` = `p`.`id`
        ;

        CREATE VIEW `empresa_dados` AS
    SELECT 
        `tc`.`TipoCadastro` AS `tipoCadastro`,
        `tite`.`id` AS `id_tipo_telefone`,
        `te`.`id` AS `id_telefone`,
        `es`.`id` AS `id_estado`,
        ...
        `e`.`NomeEmpresa` AS `NomeEmpresa`,
        `e`.`cnpj` AS `cnpj`,
        `tite`.`TipoTelefone` AS `TipoTelefone`,
        `te`.`ddd` AS `ddd`,
        `te`.`telefone` AS `telefone`,
        `c`.`CEP` AS `CEP`,
        ...
        CONCAT(`l`.`Logradouro`,
                ...
                `en`.`Numero`,
                ' ',
                `en`.`complemento`) AS `endereco_completo`,
        CONCAT(`te`.`ddd`, ' ', `te`.`telefone`) AS `telefone_completo`
    FROM
    `empresa` `e`
        JOIN `endereco` `en` ON `e`.`id` = `en`.`id_Empresa_fk`
        JOIN `cep` `c` ON `c`.`id` = `en`.`id_cep_fk`
        JOIN `cidade` `cid` ON `cid`.`id` = `c`.`id_cidade_fk`
        JOIN `estado` `es` ON `es`.`id` = `cid`.`id_estado_fk`
        JOIN `logradouro` `l` ON `l`.`id` = `en`.`id_Logradouro_fk`
        JOIN `telefone` `te` ON `te`.`id_empresa_fk` = `e`.`id`
        JOIN `tipotelefone` `tite` ON `tite`.`id` = `te`.`id_tipo_telefone_fk`
